...
public interface IndustryPartnershipRepository extends JpaRepository<IndustryPartnership, Long> {
    ...
    @Query("SELECT p FROM IndustryPartnership p WHERE p.project.university.id = :universityId OR p.challenge.assignedUniversityId = :universityId ORDER BY p.createdAt DESC")
