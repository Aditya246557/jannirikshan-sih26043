...
function AdminDashboard() {
    ...
    const validateComplaint = async (id) => {

        try {

            const remarks = window.prompt(
                "Validation remarks (optional):",
                ""
            );

            await api.put(
                `/admin/complaints/${id}/validate`,
                { remarks: remarks || "" }
            );
    ...
    const handlePrimaryAction = async (complaint) => {

        switch (complaint.status) {

            case "PENDING":
                await startReview(complaint.id);
                break;

            case "UNDER_REVIEW":
                await validateComplaint(complaint.id);
                break;
...
function ComplaintCard({
    ...
    const verifyResolution = async () => {
        if (!resolutionFile) {
            setVerificationMessage("Choose a current photo of the completed work first.");
            return;
        }
        ...
        try {
            ...
            const response = await api.post(
                `/complaints/${complaint.id}/verify-resolution`,
                formData
            );

            const passed = response.data?.resolutionVerificationStatus === "PASSED";
            setVerificationMessage(
                passed
                    ? "AI verification passed. The complaint is now resolved."
                    : "AI still detected the issue. Keep the complaint in progress and upload another photo."
            );
            ...
            setVerificationMessage(
                ...
                "Resolution verification failed. Check that the AI service is running."
            );
                        } finally {
        ...
        >
            ...
            >
                ...
                <Detail
                    ...
                    valueColor={getPriorityColor(complaint.priority)}
                />
                ...
                <Detail
                    icon="◉"
                    label="CITIZEN"
                    value={complaint.citizenEmail || "N/A"}
                />
            ...
            {complaint.status === "IN_PROGRESS" && (
                ...
                >
                    <strong style={{ color: "#7dd3fc", fontSize: "10px" }}>
                        AI Resolution Verification
                    </strong>

                    <p style={{ color: "#94a3b8", fontSize: "9px", margin: "6px 0 10px" }}>
                        Upload a fresh on-site photo. The complaint is resolved only when the issue is no longer detected.
                    </p>
                    ...
                    <div style={{ display: "flex", gap: "8px", flexWrap: "wrap", alignItems: "center" }}>
                        <input
                            ...
                            style={{ color: "#cbd5e1", fontSize: "9px", maxWidth: "220px" }}
                        />
                        ...
                        >
                            {verificationLoading ? "Verifying..." : "Verify with AI"}
                        </button>
                    </div>

                    {verificationMessage && (
                        <p style={{ color: "#cbd5e1", fontSize: "9px", margin: "9px 0 0" }}>
                            {verificationMessage}
                        </p>
                    )}
                </div>
